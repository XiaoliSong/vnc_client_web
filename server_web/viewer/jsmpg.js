...
jsmpeg.prototype.receiveSocketMessage = function( event ) {
	if(this.count==0){
		...
		else if(event.data=="error_access"){
			alert(get_query_string("to_see_id")+"对方桌面不存在");
			console.log("error_access");
			document.title=get_query_string("to_see_id")+" 的桌面不存在";
			this.client.close();
		}
		...
		else if(event.data=="control_pass"){
			allow_input=1;
			console.log("控制模式");
			document.title="正在控制 "+get_query_string("to_see_id")+" 的桌面";
		}
		else if(event.data=="view_pass"){
